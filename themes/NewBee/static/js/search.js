...
function search() {
    ...
    for (var i=0; i<postsCount; i++) {
        ...
        var keyIndex = postPlain.toLowerCase().indexOf(key.toLowerCase());
        var keyIndexTitle = postTitle.toLowerCase().indexOf(key.toLowerCase());
...
function hlHtml(oMark, key) {
    var keyIdx = oMark.toLowerCase().indexOf(key.toLowerCase());
    if (oMark && keyIdx >= 0) {
        var text = oMark;
        var newMark = '';
        

        var keyHtml = "<span class=\"key-hl\">" + text.slice(keyIdx,keyIdx+key.length) + "</span>";
        while (keyIdx >= 0) {
